...
Everything here is driven by two entry points: `python -m collect`
(acquire) and `python -m process.process` (clean → score → pool → split).
...
flowchart TD
    ...
    subgraph BEN["1b · model.evaluate.collect_benchmark &mdash; held out, never trained on"]
        direction TB
...
flowchart TD
    ...
    %% ---------------- 5. tokenizers ----------------
    FIN --> TKEN["5 · model.tokenize.train_tokenizer<br/>ByteLevel BPE · vocab 8k"]
    FIN --> TKAM["5 · model.tokenize.train_tokenizer_am<br/>Unigram / SentencePiece · vocab 8k"]
    ...
    CFG[/"model/configs/archive/*.yaml<br/>base_v4 · base_v6 · every other config, live or superseded"/] --> TRAIN
    ...
    %% ---------------- 8. evaluate ----------------
    CKPT --> EVI["8a · model.evaluate.evaluate_in_dist<br/><b>in-distribution</b> · decode + sacrebleu"]
    PRD --> EVI
    EVI --> SC1["validation BLEU"]

    CKPT --> EVB["8b · model.evaluate.evaluate_OOD<br/><b>out-of-distribution</b> · decode + sacrebleu, one checkpoint at a time<br/>--show-worst for per-sentence errors"]
...
**Why `model.evaluate.evaluate_OOD` is a separate module from `model.evaluate.evaluate_in_dist`**
rather than another split of it — all three reasons matter for the numbers being
publishable:
...
2. Both `prepare.py` (`MAX_LEN`) and `TranslationDataset` (`max_src_len`) *silently
   ...
   "FLORES devtest" means. `evaluate_OOD` truncates instead — count in equals
   count out.
3. `evaluate_in_dist` rebuilds references by decoding target ids back to text, a
   lossy tokenizer round-trip. A benchmark reference is scored exactly as it ships.

`evaluate_OOD` always re-decodes from the checkpoint rather than reading back a
stored hypothesis file, on purpose: am-en-base-v6's stored hypotheses, written
once and read back later, scored 17.35 BLEU; the same checkpoint/config/benchmark
reproduce 15.57 today under identical settings — nothing about the model changed,
the code that produced the file had simply moved on. Re-run the script whenever
you need the number rather than trusting a cached one. `--show-worst N` prints the
N lowest-sentenceBLEU pairs (source/reference/hypothesis) from that same run for
error inspection, so there's no separate file format to keep in sync with the
decoding strategy that produced it.

The Amharic **source** *is* normalized before scoring (it's the distribution
...
| I want to… | Run |
|---|---|
| Re-tune a quality cutoff | Edit `process/process.py` CONFIG, `python -m process.process` (scores come from cache; only pool re-runs) |
...
| Re-tune the NLLB LASER cutoff | `python -m collect nllb` (~3s, no network), then `python -m process.process` |
...
| Train a new run | `python -m model.train model/configs/archive/<cfg>.yaml` |
| Score in-distribution | `python -m model.evaluate.evaluate_in_dist <cfg> <ckpt> validation` |
| Score OOD | `python -m model.evaluate.evaluate_OOD <cfg> <ckpt> data/benchmarks/flores200_am_en.csv devtest` |
| See per-sentence OOD errors | add `--show-worst 20` to the `evaluate_OOD` call above |
