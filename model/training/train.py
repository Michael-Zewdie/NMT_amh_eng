...
config_path defaults to model/configs/archive/base_v6.yaml (no argparse,
matching the repo's existing plain-sys.argv precedent, e.g. collection/collect.py).
Every config, live or superseded, lives in model/configs/archive/.
"""
import sys
import time
...
from process.utils.paths import RUNS
...
def make_eval_subset_loader(cfg: Config) -> DataLoader:
    ...
    eval (loss + BLEU) — see model.evaluate.evaluate_in_dist.evaluate_loader's
    docstring for why this stays small rather than covering the whole validation split."""
