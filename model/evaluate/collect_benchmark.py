"""
model.evaluate.collect_benchmark — Pull held-out, human-translated eval sets
that must stay out of the training pool (process.py only ever globs
csv_raw/*.csv, so keeping these in data/benchmarks/ instead is what keeps them
out). Lives under model/evaluate/ rather than collect/ because what it produces
is model-evaluation input, not training data — model.evaluate.evaluate_OOD
...
Run (from the project root):
    ...
    python -m model.evaluate.collect_benchmark            # every benchmark
    python -m model.evaluate.collect_benchmark flores     # just one
...
collect/nllb.py), and openlanguagedata/flores_plus is gated behind terms acceptance.
...
or deduped. process.utils.decontaminate reads them back to strip any pooled
...
from process.utils.paths import BENCHMARKS, FLORES_FULL
